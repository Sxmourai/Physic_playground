...
class Particle:
    def __init__(self, pos: vec2, vel: vec2 = vec2(0., 0.), charge:float=1, color: pygame.Color = pygame.Color(255, 255, 255, 255), fixed: bool = False):
        self.pos = pos
        self.vel = vel
        self.color = color
        self.fixed = fixed
...
particles += [
            ...
            #   Particle(vec2(sw/2+300, sh/2)),
              Particle(vec2(sw/2+0, sh/2+1), charge=1),
              Particle(vec2(sw/2+1, sh/2-1), charge=1),
              Particle(vec2(sw/2+2, sh/2+1), charge=1),
              Particle(vec2(sw/2+3, sh/2-1), charge=1),
              Particle(vec2(sw/2+4, sh/2+1), charge=1),
              Particle(vec2(sw/2+5, sh/2-1), charge=1),
              Particle(vec2(sw/2+6, sh/2+1), charge=1),
              ]
springs.append(Spring(0, 1))
springs.append(Spring(1, 2))
springs.append(Spring(2, 3))
springs.append(Spring(3, 4))
springs.append(Spring(3, 5))
springs.append(Spring(3, 6))
# springs.append(Spring(3, 0))
# springs.append(Spring(0, 2))
# springs.append(Spring(1, 3))
# springs.append(Spring(4, 3))
# springs.append(Spring(5, 4, solid=True))
# springs.append(Spring(5, 6, solid=True))
...
while True:
    ...
    for s in springs:
        ...
        if s.solid:
            ...
            p1.pos = p1.pos - (force*10 * dir)
            p2.pos = p2.pos + (force*10 * dir)
        else:
    ...
    for p1 in particles:
        for p2 in particles:
            dist = p1.pos - p2.pos
            dist_sq = dist.dot(dist)
            if dist_sq < 0.02:continue
            force = (p1.charge * p2.charge)/math.sqrt(dist_sq)*10.
            dir = dist.normalize()
            p1.vel += force * dir
